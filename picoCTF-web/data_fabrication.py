...
admin_tid = api.team.create_team({"team_name": "admin", "password": "password", "eligibile": True})
admin_uid = api.user.create_user("admin", "admin", "admin", "admin@test.com", api.user.hash_password("password"), admin_tid, admin=True, teacher=True)
...
for user in queue:
    ...
    unlocked = [api.problem.get_problem(pid=pid) for pid in set(api.problem.get_unlocked_pids(team["tid"])) - set(api.problem.get_solved_pids(team["tid"]))]
